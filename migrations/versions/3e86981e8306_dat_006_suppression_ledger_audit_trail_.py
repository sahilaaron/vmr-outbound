...
down_revision: Union[str, Sequence[str], None] = '87cdc91ff558'
...
_suppression_reason = postgresql.ENUM(
    'OPT_OUT', 'HARD_BOUNCE', 'CUSTOMER', 'COMPETITOR', 'INTERNAL_EXCLUSION',
    'LEGAL_COMPLIANCE', 'MANUAL', name='suppression_reason', create_type=False,
)
_event_type = postgresql.ENUM(
    'CREATED', 'REACTIVATED', 'DEACTIVATED', name='suppression_event_type',
    create_type=False,
)
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['suppression_id'], ['suppressions.id'], name=op.f('fk_suppression_events_suppression_id_suppressions'), ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id', name=op.f('pk_suppression_events')),
    )
    op.create_index('ix_suppression_events_suppression_id', 'suppression_events', ['suppression_id'], unique=False)

    op.add_column('suppressions', sa.Column('created_by', sa.String(length=255), nullable=True))
    op.add_column('suppressions', sa.Column('is_active', sa.Boolean(), server_default='true', nullable=False))
    op.add_column('suppressions', sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False))
    op.drop_constraint(op.f('uq_suppressions_type_value'), 'suppressions', type_='unique')
    op.create_index('ix_suppressions_value_active', 'suppressions', ['value', 'is_active'], unique=False)
    op.create_unique_constraint('uq_suppressions_type_value_reason', 'suppressions', ['suppression_type', 'value', 'reason'])
...
def downgrade() -> None:
    ...
    op.create_unique_constraint(op.f('uq_suppressions_type_value'), 'suppressions', ['suppression_type', 'value'])
