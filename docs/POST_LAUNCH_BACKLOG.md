...
| Idea | Why deferred |
| --- | --- |
| **Script workers — plug `.py` collectors into the Research Agent** | The legacy worker seam remains available outside normal production execution. Running *arbitrary registered scripts* as workers is the v2 step. Design, mapping table and the three blocking questions are in [`RESEARCH_WORKERS.md`](RESEARCH_WORKERS.md#planned-script-workers-v2-not-built). The blockers are real: both candidate scripts would create a second Claude execution and validation route beside the reviewed primary Research boundary, so their permissions, evidence rules and provenance require a separate product decision. |
...
### Bounded follow-ups recorded by the independent hostile review (2026-08-10)

The review's H-1, M-2, M-3, L-4, L-5, L-6, L-7 and L-9 were repaired before
publication. These are the findings it explicitly classified as post-launch, plus
the one design question the M-2 decision defers. None blocks the internal Beta.

| Finding | Why deferred |
| --- | --- |
| ~~**Anonymous CORS preflight for an authenticated cross-origin client** (the M-2 decision)~~ | **Closed.** Built with extension capture authentication exactly as this entry specified: a narrow enumerated contract (`app/core/auth/extension.py`), CORS headers only, no body, no authentication implication, answered only for an approved `chrome-extension://` origin and an in-contract requested method, with its own tests in `tests/test_extension_capture_auth.py`. See `docs/HOSTED_AUTH.md`. |
| **L-8 — multiple `Cookie` headers de-authenticate** | Safe direction, and correct against request-smuggling shapes. An HTTP/2 client that splits cookies (RFC 7540 §8.1.2.5) would appear anonymous; the deployed path is nginx to uvicorn over HTTP/1.1, which recombines, so no client is affected today. The behaviour is now commented in `middleware._cookie`. |
| **L-10 — lowering `AUTH__SESSION_MAX_AGE_SECONDS` does not shorten live sessions** | A minted cookie carries its own signed `exp`, so a shortened configured lifetime applies to new sessions only. Both documented revocation levers work — allow-list removal revokes on the next request, secret rotation invalidates every live session. Documentation nuance, not a control gap. |
| **L-11 — unbounded JWKS refetch while the provider is failing** | The 30-second refresh floor applies to a live cache; a *failing* provider is retried per verification. Not attacker-reachable at scale, because reaching the verifier at all requires a valid signed transaction cookie and a successful Google code exchange first. |
...
## Captured during the Google Sheets integration

| Item | Why deferred |
| --- | --- |
| **`resolution.service._existing_company_matches` never matches a multi-word company name** | The SQL prefilter is `LOWER(name) LIKE '%<first six characters of the folded name>%'`, and folding removes spaces — so `"Kiln Systems"` folds to `"kilnsystems"`, whose first six characters `"kilnsy"` do not appear in `"kiln systems"`. A capture therefore never matches its own permanent Company by name and falls through to a logo.dev lookup it did not need. The failure direction is safe (a wasted call, never a wrong domain), which is why it has gone unnoticed. Fixing it is a change to the capture path's evidence assembly and belongs with the next change to that module; `app/services/integrations/sheets/companies.py` does the comparison without the prefilter locally and says so in its docstring. |
| **Per-integration revoke for the Sheets add-on** | The add-on's credential is minted by Google per execution and never stored, so disabling the VMR account already stops it on the next call. A revoke list would be a new table for a guarantee that is already held. Revisit if a second integration ever holds a durable grant. |
| **Per-account rate limiting across requests** | `SHEETS__MAX_BATCH_ROWS` bounds one request, which is the bound the first user needs. A budget spanning requests is a 100-user question with no operating evidence behind it yet. |
| **Marketplace publication, domain-wide install, billing** | Not required for the first user; each is a distribution decision rather than a product one. |
| **A batch table and `GET /integrations/sheets/batches/{id}`** | The add-on already holds one identifier per row, so asking about exactly those identifiers is both smaller and more precise. Revisit only if a caller ever needs to enumerate a batch it did not submit. |
