...
def test_a_genuine_setup_condition_is_kept_separate_from_a_machine_failure(
    ...
    body = client.get(_campaign_url(scenario)).text
    # The customer-owned condition is said in the customer's words, and it is
    # said deterministically. This Campaign is paused *and* has a failed Research
    # Agent, so both an administrator cause and a customer cause are live at
    # once; the customer's own pause is the one that must be reported, because
    # it is the one they can act on. Accepting either sentence here would let the
    # page tell a customer their own pause was an administrator hold — which is
    # false, and hides the Resume they own.
